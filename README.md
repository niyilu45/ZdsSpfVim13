...
- Python 文件保存后的 Syntastic 语法检查会按 UTF-8 或文件头部的 `coding:` 声明读取源码，不再错误地使用系统 GBK 编码而触发 `UnicodeDecodeError`。
...
- Vim 启动或编码验证失败时，安装器会把 Vim 路径、退出码、实际编码设置、`v:errmsg` 和插件原始输出完整打印在当前窗口中。
- 双击批处理安装时，失败窗口会停住，不需要再去日志文件里找错误。
- 同一份信息还会额外保存到 `%USERPROFILE%\spf13-vim-install-error.log`；该路径固定，并且不会在自动回滚时被删除。
